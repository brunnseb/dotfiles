...
local tabs = require("external.machina.tabs")
local geoms = require("external.machina.geoms")
local helpers = require("external.machina.helpers")
...
local function screen_info(s)
	...
	return focused_screen, workarea, selected_tag, layout, focused_client
end
...
local function get_region(region_ix, s)
	local s = s or awful.screen.focused()
	local focused_screen, workarea, selected_tag, layout, focused_client = screen_info(s)
	...
	if layout.machi_get_instance_data then
		...
		table.sort(machi_regions, function(a1, a2)
			return a1.id > a2.id
...
local function get_client_info(c)
	...
	if not source_client then
		return {}
	end
	--|flow control

	if source_client.x < 0 or source_client.y < 0 then
		outofboundary = true
	...
	if not active_region then
		...
		if source_client.floating then
			if
				regions[active_region].width - source_client.width ~= 0
...
local function get_clients_in_region(region_ix, c, s)
	...
	if not active_region then
		for i, a in ipairs(regions) do
			if
				a.x <= source_client.x
				and source_client.x < a.x + a.width
				and a.y <= source_client.y
				and source_client.y < a.y + a.height
			then
				active_region = i
			end
		end
	...
	if #region_clients == 0 then
		for i, w in ipairs(s.clients) do
			if not w.floating then
				if
					math.abs(regions[active_region].x - w.x) <= 5
					and math.abs(regions[active_region].y - w.y) <= 5
					-- or
					-- (
...
local function expand_horizontal(direction)
	return function()
		...
		if not target and direction ~= "center" then
			return
		end -- flow control
...
local function floating_signal(c)
	if not global_client_table[c.window] then
		return
	end
