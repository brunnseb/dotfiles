return {
  ...
  },
  {
    ...
    event = 'VeryLazy',
    keys = {
      { '<leader>lr', '<cmd>lua require("timber.actions").clear_log_statements({ global = false })<CR>', desc = 'Remove logs' },
      { '<leader>lt', '<cmd>lua require("timber.actions").toggle_comment_log_statements({ global = false })<CR>', desc = 'Toggle logs' },
    },
    opts = {
      log_templates = {
        default = {
